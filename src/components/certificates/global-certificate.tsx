...
export default function GenerarCertificadoGlobal({ titulo, open, onOpenChange }: GenerarCertificadoModalProps) {
  const [pdfUrl, setPdfUrl] = useState<string | null>(null)

  useEffect(() => {
    if (open) {
      generarCertificadoPDF(titulo).then(url => setPdfUrl(url))
    } else {
      setPdfUrl(null)
    }
  }, [open, titulo])
...
async function generarCertificadoPDF(titulo: string): Promise<string> {
  ...
  const fullName = "Juan Carlos Pérez Gómez"
  const sacerdote = "Ignacio Caizabanda Jerez"
  const nombresPadre = "Pedro Pérez"
  const nombresMadre = "María Gómez"
  const padrino = "Luis Martínez"
  const fechaBautismo = new Date("2015-03-15")
  const dia = fechaBautismo.getDate()
  const mes = fechaBautismo.toLocaleString("es-ES", { month: "long" })
  const anio = fechaBautismo.getFullYear()
  ...
  drawCenteredText(`PARTIDA DE ${titulo.toUpperCase()}`, y, 18, fontBold); y -= 50
  ...
  y = drawParagraphWithBoldData([
    "el Rvdo. Padre ", { text: sacerdote, bold: true },
    " realizo solemnemente el sacramentro a: ", { text: fullName, bold: true }, "."
  ], MARGIN_X, y)

  y = drawParagraphWithBoldData([
    "Hijo(a) de ", { text: nombresPadre, bold: true },
    " y de ", { text: nombresMadre, bold: true }, "."
  ], MARGIN_X, y)

  y = drawParagraphWithBoldData([
    "Fueron padrinos: ", { text: padrino, bold: true }, "."
  ], MARGIN_X, y)
